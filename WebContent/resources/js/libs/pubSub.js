...
define(['jquery'], function($) {
	...
	$.extend({
	  ...
	  $.publish = function() {
		  // arguments[2] is an object from the window
		  // Can be extendes without arguments[2] but to /<iframe-name>/func
		  if(arguments[2]) {
			  var _message = {
					topic: arguments[0],
					msgData: arguments[1]
			  ...
			  crossFrame.send(
					 arguments[2].proxyUrl, // proxyUrl 
					 arguments[2].targetIframe, //targetIframe
					 $.stringify(_message) //message
			  );
		  }
		  else {
	  ...
	  $.subscribe("iframeMessage", function (e, message, domain, uri) {
		  	message = $.parseJSON(message);
		    $.publish(message.topic, [message.msgData]);
	  });
